...
def procesar_peticion(conexion, direccion):
    try:
        ...
        if datos:
            print(f"[Worker 1] Consulta recibida para ID: {datos} desde {direccion}")
            texto_recuperado = consultar_base_datos(datos)

            respuesta = f"[Respuesta de Worker 1] ID {datos}: {texto_recuperado}"
...
def iniciar_servidor():
    ...
    servidor.listen(5)
    print(f"Servidor 1 (Worker) activo en puerto {puerto}...")
